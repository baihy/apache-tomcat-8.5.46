...
final class StandardWrapperValve
    ...
    public final void invoke(Request request, Response response)
        ...
        requestCount.incrementAndGet();  //  通过原子变量实现自增加的原子操作 CAS（比较和交换）
       ...
       // 这就调用的Servlet对象
        ...
        // 检车Servlet是否被标记为不可用
        if (!unavailable && wrapper.isUnavailable()) { container.getLogger().info(sm.getString("standardWrapper.isUnavailable", wrapper.getName()));
        ...
        try {
            if (!unavailable) {
                //这里就是第一次访问Servlet的时候， 获取Servlet的对象
                ...
                if (context.getSwallowOutput()) {
                    try {
                        ...
                        if (request.isAsyncDispatching()) {
                            ...
                            // 这里执行的filterChain的doFilter的方法，并不是Filter的doFilter方法
